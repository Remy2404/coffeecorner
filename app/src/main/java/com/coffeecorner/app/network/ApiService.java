...
import com.coffeecorner.app.models.CartAddRequest;
...
public interface ApiService {
    ...
    @FormUrlEncoded
    @POST("auth/forgot-password")
    Call<ApiResponse<Void>> requestPasswordReset(@Field("email") String email);    @FormUrlEncoded
    @POST("auth/firebase-auth")
    Call<AuthResponse> authenticateWithFirebase(@Field("firebase_token") String firebaseToken);
    ...
    Call<ApiResponse<Void>> changePassword(
            ...
            @Field("newPassword") String newPassword); // Cart endpoints - JWT authenticated

    @GET("cart")
    Call<ApiResponse<List<CartItem>>> getCart();

    // Legacy userId-based endpoint (keep for compatibility)
    @GET("cart/{userId}")
    Call<ApiResponse<List<CartItem>>> getCart(@Path("userId") String userId);

    // JWT-authenticated endpoint that expects JSON body
    @POST("cart/add")
    Call<ApiResponse<List<CartItem>>> addToCart(@Body CartAddRequest cartAddRequest);

    // Legacy form-encoded endpoint (keep for compatibility)
    @FormUrlEncoded
    @POST("cart/add")
    Call<ApiResponse<List<CartItem>>> addToCartFormEncoded(
    ...
    Call<ApiResponse<Void>> clearCart(@Path("userId") String userId); // Order endpoints
