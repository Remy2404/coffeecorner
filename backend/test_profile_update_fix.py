        ...
        try:
            ...
            if update_response.status_code == 200:
                ...
                if response_data.get("success"):
                    ...
                    logger.info(
                        f"Updated user data: {json.dumps(response_data['data'], indent=2)}"
                    )
                    return True
                else:
                    logger.error(
                        f"❌ Profile update failed: {response_data.get('message', 'Unknown error')}"
                    )
                    return False
            else:
                logger.error(
...
async def test_both_endpoints():
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            return (
                auth_response.status_code == 200 and users_response.status_code == 200
            )
...
async def main():
    ...
    logger.info("=" * 50)
    logger.info(
        f"Profile Update Endpoint Test: {'✅ PASS' if test1_result else '❌ FAIL'}"
    )
