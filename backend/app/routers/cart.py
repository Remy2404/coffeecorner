...
async def get_cart(current_user: UserResponse = Depends(get_current_user)):
    ...
    cart_items = await CartService.get_user_cart(current_user.id)
    return ApiResponse(
...
async def add_to_cart(
    cart_item: CartItemAdd, current_user: UserResponse = Depends(get_current_user)
):
    """Add item to cart"""
    item = await CartService.add_to_cart(current_user.id, cart_item)
    return ApiResponse(
...
async def update_cart_item(
    ...
    item = await CartService.update_cart_item(current_user.id, item_id, update_data)
    return ApiResponse(
...
async def remove_from_cart(
    item_id: str, current_user: UserResponse = Depends(get_current_user)
):
    """Remove item from cart"""
    success = await CartService.remove_from_cart(current_user.id, item_id)
    if not success:
        raise HTTPException(
...
async def get_cart_total(current_user: UserResponse = Depends(get_current_user)):
    """Get cart total"""
    total = await CartService.get_cart_total(current_user.id)
    return ApiResponse(
        success=True,
